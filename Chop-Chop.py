...
class PlannerStartPage (tk.Frame):
    ...
    def no_defs(self):
        ...
        c = conn.cursor()
        c.execute('select meal from RecipeList')
        self.rmeal_list_after_def = c.fetchall()
        conn.close()
        self.filter()
    ...
    def some_defs(self):
        ...
        if self.ryes_dict and not self.rno_dict:
            self.only_yes()
        if self.rno_dict and not self.ryes_dict:
            self.only_no()
        if self.rno_dict and self.ryes_dict:
            self.both_yes()
    ...
    def only_yes(self):
        ...
        for key in self.ryes_dict:
            ...
            ryes = c.fetchall()
            rfully = [i for i in rfully if i in ryes]
        self.rmeal_list_after_def = rfully
        conn.close()
        self.filter()
    ...
    def only_no(self):
        ...
        for key in self.rno_dict:
            ...
            rno = c.fetchall()
            rfulln = [i for i in rfulln if i not in rno]
        self.rmeal_list_after_def = rfulln
        conn.close()
        self.filter()
    ...
    def both_yes(self):
        ...
        for key in self.ryes_dict:
            ...
            rfully = [i for i in rfully if i in ryes]
        self.rmeal_list_after_def = rfully
        if not self.rmeal_list_after_def:
            conn.close()
            self.filter()
        else:
            c.execute("select meal from RecipeList")
            ...
            no_list = rfulln
            if not no_list:
                self.rmeal_list_after_def = no_list
                conn.close()
                self.filter()
            else:
                self.rmeal_list_after_def = [i for i in self.rmeal_list_after_def if i in no_list]
                conn.close()
                self.filter()
    ...
    def filter(self):
        ...
        exp = [i for i in self.rset_of_meals]
        expo = str(exp)
    ...
    def rcalc_results(self, **kwargs):
        rand_exploded_raw_meal_list = self.rset_of_meals
...
class AddDefault (tk.Frame):
    ...
    def make_def_screen(self, controller, x, parent):
        ...
        for item in self.names1:
            ...
            self.x = [it for t in self.options_list for it in t]
            self.handler_dict = {}
            s = shelve.open('test_shelf.db')
...
class CustomPlan(tk.Frame):
    ...
    def make_default_dict(self, controller, parent):
        ...
        if not self.def_dict:
            self.cno_defs(controller, parent)
        else:
            self.csome_defs(controller, parent)

    def cno_defs(self, controller, parent):
        conn = sqlite3.connect('master.db')
        c = conn.cursor()
        c.execute('select meal from RecipeList')
        self.meal_list_after_def = c.fetchall()
        conn.close()
        self.cfilter(controller, parent)

    def csome_defs(self, controller, parent):
        for citem in self.def_dict:
            for opt in self.def_dict[citem]:
        ...
        if self.yes_dict and not self.no_dict:
            self.conly_yes(controller, parent)
        if self.no_dict and not self.yes_dict:
            self.conly_no(controller, parent)
        if self.no_dict and self.yes_dict:
            self.cboth_yes(controller, parent)

    def conly_yes(self, controller, parent):
        conn = sqlite3.connect('master.db')
        c = conn.cursor()
        c.execute("select meal from RecipeList")
        fully = c.fetchall()
        for key in self.yes_dict:
            ...
            yes = c.fetchall()
            fully = [i for i in fully if i in yes]
        self.meal_list_after_def = fully
        conn.close()
        self.cfilter(controller, parent)

    def conly_no(self, controller, parent):
        conn = sqlite3.connect('master.db')
        c = conn.cursor()
        c.execute("select meal from RecipeList")
        fulln = c.fetchall()
        for key in self.no_dict:
            ...
            no = c.fetchall()
            fulln = [i for i in fulln if i not in no]
        self.meal_list_after_def = fulln
        conn.close()
        self.cfilter(controller, parent)

    def cboth_yes(self, controller, parent):
        conn = sqlite3.connect('master.db')
        c = conn.cursor()
        c.execute("select meal from RecipeList")
        fully = c.fetchall()
        for key in self.yes_dict:
            ...
            fully = [i for i in fully if i in yes]
        self.meal_list_after_def = fully
        if not self.meal_list_after_def:
            conn.close()
            self.cfilter(controller, parent)
        else:
            c.execute("select meal from RecipeList")
            ...
            no_list = fulln
            if not no_list:
                self.meal_list_after_def = no_list
                conn.close()
                self.cfilter(controller, parent)
            else:
                self.meal_list_after_def = [i for i in self.meal_list_after_def if i in no_list]
                conn.close()
                self.cfilter(controller, parent)

    def cfilter(self, controller, parent):
        exploded_def_meals = ([item for t in self.meal_list_after_def for item in t])
        self.set_of_meals = set(exploded_def_meals)
    ...
    def pop_upper(self, parent, controller, **kwargs):
        ...
        for k in kwargs:
            if not kwargs[k]:
                ghi+=1
        if ghi ==0:
            self.go(parent, controller, **kwargs)

    def go(self, parent, controller, **kwargs):
        self.day_attr_val_dict_hold = {}
        self.no_days = {}
        self.list_no_days = []
        colourd = ['red', 'black', 'green', 'red', 'black', 'green', 'red']
